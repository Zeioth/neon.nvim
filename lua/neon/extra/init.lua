...
function M.setup()
  ...
  -- neon.setup({ plugins = { all = true } })
  for _, extra in ipairs(names) do
    local info = M.extras[extra]
    local plugin = require("neon.extra." .. extra)
    for style, style_name in pairs(styles) do
      local colors, groups, opts = neon.load({ style = style, plugins = { all = true } })
      local fname = extra
        ...
        .. "."
        .. info.ext
      colors["_upstream_url"] = "https://github.com/folke/neon.nvim/raw/main/extras/" .. fname
      colors["_style_name"] = "Tokyo Night" .. style_name
      colors["_name"] = "neon_" .. style
      colors["_style"] = style
      print("[write] " .. fname)
