...
function M.extras()
  local Extra = require("neon-cherry-kiss.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/neon-cherry-kiss/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/neon-cherry-kiss/config.lua", "\n(--@class neon-cherry-kiss%.Config.-\n})")
